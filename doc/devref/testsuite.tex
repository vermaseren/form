...
(following the widely used regex-syntax). There are three functions to
facilitate things: \C{result()}, \C{Pattern()}, and \C{ExactPattern()}.
\C{result()} takes a string being the name of an expression and returns a string
that only contains the lines belonging to the last output of this expression.
\C{Pattern()} transforms special characters in the given string, removes
unnecessary whitespace, and returns the string as a pattern with multi-line
matching enabled. Since \FORM\
expressions usually contain a lot of special characters like +, *, ., etc. they
cannot not be simply used as a pattern. \C{Pattern()} transforms these
characters automatically into the correct regex equivalent, e.g. + becomes
\textbackslash +.
With it, a \FORM\ expression can be directly given as an argument and used in a
pattern matching (see example). \C{ExactPattern()} does not treat whitespaces
and line breaks in a special way as \C{Pattern()} does and can therefore be used
when a exact comparison is required (if for example a bug in the output
functions of \FORM\ had caused some whitespace or line breaks to be missing and
a test case were required to check for this behavior).
...
\C{ExactPattern()}) we can be sloppy about the indentation and the whitespaces.
