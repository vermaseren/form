...
on' but this should be avoided in the future.}
...
cost of some CPU time. The digit indicates the compression level. Zero means
no compression and 9 is the highest level. The default level is 6. Above that
the compression becomes very slow and doesn't gain very much extra.
This setting is the default if FORM is not compiled with Zstd support.}

\leftvitem{3.5cm}{compress,zstd\index{zstd}}
\rightvitem{13cm}{This option is functionally the same as `On compress,gzip;'
but uses Zstd as the compression library, which provides better performance
both in terms of CPU time and compression ratio. This setting is the default
if FORM is compiled with Zstd support.}
