...
\item[-M] Put the PID (process identifier) in the name of the temporary 
     ...
     the make -j option).
%\item[-n] This should be followed immediately by a number. The 
%     number indicates the number of slave processes for ParFORM when 
%     ParFORM isn't using MPI for communication. All other 
%     versions of FORM ignore this parameter.
%%%% Maybe this should become -w as well.
