...
VERSION\_     &\index{VERSION\_} The current version as the 3 in 3.2.\\
SUBVERSION\_  &\index{SUBVERSION\_} The sub-version as the 2 in 3.2.\\
NAMEVERSION\_ &\index{NAMEVERSION\_} For special versions that are 
                experimental. Otherwise empty.\\
NAME\_        &\index{NAME\_} The name of the program file.\\
DATE\_        &\index{DATE\_} The date of the current run.\\
CMODULE\_     &\index{CMODULE\_} The number of the current module.\\
SHOWINPUT\_   &\index{SHOWINPUT\_} If input listing is on: 1, if off: 0.
\end{tabular}
...
should be done with it. For this FORM is equiped with what is called the 
...
brackets fullfil the same role, as one can nest these brackets of course. 
Operators are:
\begin{description}
...
the directories indicated the path\index{path} variable in either the setup 
...
\#define\index{\#define} instruction on page \ref{predefine}) apply. The 
...
\begin{verbatim}
   ...
   F2 =
      7;
\end{verbatim}
The parameter c will be substituted inmediately when var1 is defined. In 
...
\#enddo\index{\#enddo} instruction. All code inbetween these two 
...
\begin{description}
\item[\#do lvar = i1,i2] The parameters i1 and i2 should be integers. The 
...
printed as well. If one needs messages without this clear markation, one 
...
\noindent in which\index{\#undefine} name refers to the name of the 
...
create a certain environment. These environments cannot be mixed. This 
