...
used it loads this buffer and the contents stay around untill either a 
...
\item[Horner=(Occurrence $|$ MCTS)] Determines whether an 
  occurrence order\index{occurrence order} Horner scheme is used
  or whether MCTS\index{MCTS}\index{Monte Carlo tree search} is employed to
  find Horner schemes.
...
printed in reverse order with the the string "id " in front. This makes 
