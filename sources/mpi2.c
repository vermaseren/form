...
{  
	...
	if ( ( PF_group_array = 
             (MPI_Group*)Malloc1(PF.numtasks*sizeof(MPI_Group),"PF_group_array")) == 0 ) {
    	MesPrint("PF_group_array allocation error");
	    exit(-1);
	} 

	if ( ( PF_comm_array = 
             (MPI_Comm*)Malloc1(PF.numtasks*sizeof(MPI_Comm),"PF_comm_array")) == 0 ) {
...
{  
	...
	if ( size > AM.shmWinSize*sizeof(WORD) ) {
		MesPrint("PF_Put_origin: Enlarge shared window size ...");
/*
...
*
	...
	r = MPI_Put(s->buff[a], size, PF_WORD, to, 0, size, PF_WORD, PF_shared_win);
...
/*
	...
	Synchronization for put operation on target
*/
