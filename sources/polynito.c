...
*/
	if ( *p2 == 4 && p2[4] == 0 ) {
		...
		if ( (LONG)(size*sizeof(WORD)) >= AM.MaxTer ) {
			MLOCK(ErrorMessageLock);
...
*/
	...
	while ( success < MINIMUMSUCCESRATE ) {
		...
		prime = NextPrime(BHEAD numprime);
		if ( ( ( min1 = PolyModSubsVector(Poly1,config,numvars,prime,
					-1,plist1[plist1[1]],&(AN.polymod1)) ) >= 0 ) &&
		     ( ( min2 = PolyModSubsVector(Poly2,config,numvars,prime,
...
*/
	for ( i = 2; i < plist1[1]; i += 2 ) {
		...
		while ( success < MINIMUMSUCCESRATE ) {
			config = PolyGetConfig(BHEAD numvars);
			prime = NextPrime(BHEAD numprime);
			if ( ( PolyModSubsVector(Poly1,config,numvars,prime,
						plist1[i],plist1[i+1],&(AN.polymod1)) == 0 ) &&
			     ( PolyModSubsVector(Poly2,config,numvars,prime,
...
WORD PolyModSubsVector(WORD *Poly, WORD *values, WORD num, WORD prime, WORD numsym,
